...
// win32 compatibility
#ifdef _WIN32
 #if defined(SWA_BUILDING_SHARED)
  #define SWA_EXTERN __declspec(dllexport)
 #elif defined(SWA_USING_SHARED)
  #define SWA_EXTERN __declspec(dllimport)
 #else
  #define SWA_EXTERN
 #endif
#endif
...
// `swa_display_destroy`.
SWA_EXTERN struct swa_display* swa_display_autocreate(void);

// Destroys and frees the passed display. It must not be used after this.
SWA_EXTERN void swa_display_destroy(struct swa_display*);
...
SWA_EXTERN bool swa_display_dispatch(struct swa_display*, bool block);
...
SWA_EXTERN void swa_display_wakeup(struct swa_display*);

// Returns the capabilities of the passed display.
// See `swa_display_cap` for details.
SWA_EXTERN enum swa_display_cap swa_display_capabilities(struct swa_display*);
...
SWA_EXTERN const char** swa_display_vk_extensions(struct swa_display*, unsigned* count);


// Returns whether the given key is currently pressed.
// Only valid if the display has the 'keyboard' capability.
SWA_EXTERN bool swa_display_key_pressed(struct swa_display*, enum swa_key);
...
// Only valid if the display has the 'keyboard' capability.
SWA_EXTERN const char* swa_display_key_name(struct swa_display*, enum swa_key);

// Returns all currently active keyboard modifiers.
// Only valid if the display has the 'keyboard' capability.
SWA_EXTERN enum swa_keyboard_mod swa_display_active_keyboard_mods(struct swa_display*);

// Returns the `swa_window` that currently has keyboard focus or NULL
// if there is none.
// Only valid if the display has the 'keyboard' capability.
SWA_EXTERN struct swa_window* swa_display_get_keyboard_focus(struct swa_display*);


// Returns whether the given mouse button is currently pressed.
// Only valid if the display has the 'mouse' capability.
SWA_EXTERN bool swa_display_mouse_button_pressed(struct swa_display*, enum swa_mouse_button);
...
// Only valid if the display has the 'mouse' capability.
SWA_EXTERN void swa_display_mouse_position(struct swa_display*, int* x, int* y);

// Returns the `swa_window` that the mouse is currently over or NULL
// if there is none.
// Only valid if the display has the 'mouse' capability.
SWA_EXTERN struct swa_window* swa_display_get_mouse_over(struct swa_display*);
...
SWA_EXTERN struct swa_data_offer* swa_display_get_clipboard(struct swa_display*);
...
// Only valid if the display has the 'clipboard' capability.
SWA_EXTERN bool swa_display_set_clipboard(struct swa_display*, struct swa_data_source*, void* trigger_event_data);
...
SWA_EXTERN bool swa_display_start_dnd(struct swa_display*, struct swa_data_source*, void* trigger_event_data);
...
SWA_EXTERN struct swa_window* swa_display_create_window(struct swa_display*,
	const struct swa_window_settings*);

// window api
SWA_EXTERN void swa_window_destroy(struct swa_window*);
SWA_EXTERN enum swa_window_cap swa_window_get_capabilities(struct swa_window*);
SWA_EXTERN void swa_window_set_min_size(struct swa_window*, unsigned w, unsigned h);
SWA_EXTERN void swa_window_set_max_size(struct swa_window*, unsigned w, unsigned h);

// Shows or hides the window.
// Only valid if the window has the window has the 'visibility' capability.
SWA_EXTERN void swa_window_show(struct swa_window*, bool show);
...
SWA_EXTERN void swa_window_set_size(struct swa_window*, unsigned w, unsigned h);
...
SWA_EXTERN void swa_window_set_position(struct swa_window*, int x, int y);
...
SWA_EXTERN void swa_window_set_cursor(struct swa_window*, struct swa_cursor cursor);
...
SWA_EXTERN void swa_window_refresh(struct swa_window*);
...
SWA_EXTERN void swa_window_surface_frame(struct swa_window*);
...
SWA_EXTERN void swa_window_set_state(struct swa_window*, enum swa_window_state);
...
SWA_EXTERN void swa_window_begin_move(struct swa_window*, void* trigger_event_data);
...
SWA_EXTERN void swa_window_begin_resize(struct swa_window*, enum swa_edge edges,
...
SWA_EXTERN void swa_window_set_title(struct swa_window*, const char* utf8);
...
SWA_EXTERN void swa_window_set_icon(struct swa_window*, const struct swa_image* image);
...
SWA_EXTERN bool swa_window_is_client_decorated(struct swa_window*);

// The listener object must remain valid until it is changed or the window is
// destroyed.
SWA_EXTERN void swa_window_set_listener(struct swa_window*, const struct swa_window_listener*);
SWA_EXTERN const struct swa_window_listener* swa_window_get_listener(struct swa_window*);
...
SWA_EXTERN void swa_window_set_userdata(struct swa_window*, void* data);

// Retrieves the data previously set with `swa_window_set_userdata`.
SWA_EXTERN void* swa_window_get_userdata(struct swa_window*);
...
SWA_EXTERN bool swa_window_get_vk_surface(struct swa_window*, void* vkSurfaceKHR);

// Only valid if the window was created with surface set to `swa_surface_gl`.
SWA_EXTERN bool swa_window_gl_make_current(struct swa_window*);
SWA_EXTERN bool swa_window_gl_swap_buffers(struct swa_window*);
SWA_EXTERN bool swa_window_gl_set_swap_interval(struct swa_window*, int interval);
...
SWA_EXTERN bool swa_window_get_buffer(struct swa_window*, struct swa_image*);
...
// call to `get_buffer`.
SWA_EXTERN void swa_window_apply_buffer(struct swa_window*);
...
SWA_EXTERN void swa_data_offer_destroy(struct swa_data_offer*);
...
SWA_EXTERN bool swa_data_offer_formats(struct swa_data_offer*, swa_formats_handler cb);
...
SWA_EXTERN bool swa_data_offer_data(struct swa_data_offer*, const char* format,
	swa_data_handler cb);
...
SWA_EXTERN void swa_data_offer_set_preferred(struct swa_data_offer*, const char* format,
	enum swa_data_action action);

// Returns the currently selected action for the given data offer.
SWA_EXTERN enum swa_data_action swa_data_offer_action(struct swa_data_offer*);

// Returns all supported actions by this offer.
SWA_EXTERN enum swa_data_action swa_data_offer_supported_actions(struct swa_data_offer*);
SWA_EXTERN void swa_data_offer_set_userdata(struct swa_data_offer*, void*);
SWA_EXTERN void* swa_data_offer_get_userdata(struct swa_data_offer*);
...
SWA_EXTERN void swa_window_settings_default(struct swa_window_settings*);

// Returns the size of one pixel in the given formats in bytes.
SWA_EXTERN unsigned swa_image_format_size(enum swa_image_format);
...
SWA_EXTERN struct swa_image swa_convert_image_new(const struct swa_image* src,
...
SWA_EXTERN void swa_convert_image(const struct swa_image* src, const struct swa_image* dst);

// Returns the corresponding image format with reversed component order.
// Example: argb32 will be mapped to bgra32.
SWA_EXTERN enum swa_image_format swa_image_format_reversed(enum swa_image_format);
...
SWA_EXTERN enum swa_image_format swa_image_format_toggle_byte_word(enum swa_image_format);
